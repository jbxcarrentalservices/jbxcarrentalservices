...
// -------------------------
// META PIXEL BOOKING FUNNEL
// -------------------------
// Tracks progress through the booking wizard without sending
// customer name, phone number, email, address, or other PII.

function trackBookingEvent(eventName){
...
let currentStep = 1;
let bookingStep2Tracked = false;
let bookingStep3Tracked = false;
...
function showStep(step){
    ...
    // Track each funnel step only once per page visit.
    if(step === 2 && !bookingStep2Tracked){

        bookingStep2Tracked = true;
        trackBookingEvent("BookingStep2");

    }

    if(step === 3 && !bookingStep3Tracked){

        bookingStep3Tracked = true;
        trackBookingEvent("BookingStep3");
...
// Booking page visit.
trackBookingEvent("BookingPageView");
...
document.getElementById("bookMessenger").onclick = function(){
    ...
    if(typeof fbq === "function"){
        fbq("track", "Lead");
        fbq("trackCustom", "BookingSubmitted");
    }
